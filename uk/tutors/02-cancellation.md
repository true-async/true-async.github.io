...
`delay` викинула виняток `Async\AsyncCancellation`. Те саме станеться і зі звичайним `sleep(1)`:
під TrueAsync `sleep()` теж стає асинхронним і теж є точкою скасування — він викине
`Async\AsyncCancellation` так само, як `delay`.
