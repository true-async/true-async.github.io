...
- Завдання, що очікують у черзі (ще не взяті робочим потоком), **негайно відхиляються** — відповідні об'єкти `Future` переходять у стан відхилення з `ThreadPoolException` (а в режимі `coroutine: true` — з `CancellationException`).
- У **звичайному режимі** (`coroutine: false`) завдання, що вже виконуються в робочих потоках, доводяться до завершення поточного завдання — примусово перервати PHP-код в OS-потоці неможливо.
- У **режимі `coroutine: true`** in-flight завдання **реально скасовуються** (починаючи з TrueAsync 0.7.0): атомарний прапор `cancel_requested` виставляється до закриття каналу, воркер викликає `ZEND_ASYNC_SCOPE_CANCEL(pool_scope, NULL, false, false)`, а `AFTER_MAIN` каскадно скасовує всі child-scope'и запущених завдань.
