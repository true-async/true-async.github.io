...
- `Async\OperationCanceledException` — якщо спрацював токен скасування (включно з тайм-аутом). Оригінальний виняток із токена доступний через `$e->getPrevious()` (наприклад, `TimeoutException` при використанні `timeout()`).
