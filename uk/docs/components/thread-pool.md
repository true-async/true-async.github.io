...
```php
// автодетект за кількістю CPU
$pool = new ThreadPool();

// явна кількість воркерів
$pool = new ThreadPool(workers: 4);

// + розмір черги
$pool = new ThreadPool(workers: 4, queueSize: 64);

// + bootloader, який виконується в кожному воркері один раз
$pool = new ThreadPool(
    workers: 4,
    queueSize: 64,
    bootloader: function () {
...
// + coroutine-mode: задача стартує як корутина в per-worker pool scope
$pool = new ThreadPool(workers: 4, coroutine: true);
```

| Параметр      | Тип         | Призначення                                                          | За замовчуванням  |
|---------------|-------------|----------------------------------------------------------------------|-------------------|
| `$workers`    | `int`       | Кількість робочих потоків. `0` — автодетект через `available_parallelism()` | `0`         |
| `$queueSize`  | `int`       | Максимальна довжина черги очікуючих завдань                          | `workers × 4`     |
| `$bootloader` | `?\Closure` | Per-worker startup hook (див. нижче)                                 | `null`            |
| `$coroutine`  | `bool`      | Запускати кожне завдання як корутину (див. нижче)                    | `false`           |
| `$concurrency`| `int`       | Ліміт одночасних корутин на воркер (лише при `coroutine: true`)      | `0` (без ліміту)  |

Усі робочі потоки запускаються **одразу при створенні** пулу. Це невеликі «попередні» витрати,
зате наступні виклики `submit()` не несуть накладних витрат на запуск потоку.
...
### Автодетект кількості воркерів

Коли `workers: 0` (або параметр опущено), пул бере значення з
[`Async\available_parallelism()`](/uk/docs/reference/available-parallelism.html). Ця функція
враховує `cgroup`-квоти, affinity і контейнерні ліміти — на Kubernetes-pod'і з `cpu.max=2`
ви отримаєте 2, а не фізичне число ядер хоста.

### Bootloader

Замикання `$bootloader` один раз глибоко копіюється і виконується в **кожному** воркері перед
основним циклом обробки завдань. Ідеальне місце для autoload, прогріву пулів з'єднань і
прекомпіляції opcache — усього, що інакше запускалося б усередині кожного `submit()`.

Якщо bootloader кине виняток, увесь пул вважається невдалим: воркер не стартує, а
помилка піднімається в батька.

### Режим корутин

З `coroutine: true` кожне завдання виконується **як корутина** у своїй дочірній області (scope),
яка, своєю чергою, вкладена в спільну область пулу воркера. Усередині завдання можна робити `await`,
використовувати `Channel`-и, IO і `spawn` — усе без блокування самого воркера.

```php
$pool = new ThreadPool(workers: 4, coroutine: true);

$pool->submit(function () {
    // звичайні блокуючі виклики в цьому режимі коректно паркують корутину,
    // а не блокують OS-потік
    $rows = (new PDO('mysql:...'))->query('SELECT ...')->fetchAll();
    return $rows;
});
```

`$concurrency` обмежує кількість одночасно живих корутин усередині **одного** воркера.
