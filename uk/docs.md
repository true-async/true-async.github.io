...
<p class="lm-hint" id="lmHint">Наведіть курсор на вузол для деталей. Натисніть, щоб перейти до документації.</p>
...
<button class="sh-close" id="lmShClose" aria-label="Закрити">&times;</button>
</div>
...
<a class="sh-btn" id="lmShBtn" href="#">Відкрити документацію &rarr;</a>
</div>
<a class="lm-link" href="/uk/interactive/learning-map.html">Повна інтерактивна версія &rarr;</a>
<script>
(function(){
var GRP = {
    primitives:   {color:'#6B58FF',bg:'rgba(107,88,255,0.12)',border:'rgba(107,88,255,0.50)',arrow:'url(#lmArrPurple)',label:'Базові примітиви'},
    sync:         {color:'#2563EB',bg:'rgba(37,99,235,0.12)',border:'rgba(37,99,235,0.50)',arrow:'url(#lmArrBlue)',label:'Синхронізація'},
    cancellation: {color:'#DC2626',bg:'rgba(220,38,38,0.12)',border:'rgba(220,38,38,0.50)',arrow:'url(#lmArrRed)',label:'Скасування'},
    structural:   {color:'#0891B2',bg:'rgba(8,145,178,0.12)',border:'rgba(8,145,178,0.50)',arrow:'url(#lmArrTeal)',label:'Стр. конкурентність'},
    context:      {color:'#8B5CF6',bg:'rgba(139,92,246,0.12)',border:'rgba(139,92,246,0.50)',arrow:'url(#lmArrViolet)',label:'Контекст'},
    iterate:      {color:'#F59E0B',bg:'rgba(245,158,11,0.12)',border:'rgba(245,158,11,0.50)',arrow:'url(#lmArrAmber)',label:'iterate()'},
    resources:    {color:'#16A34A',bg:'rgba(22,163,74,0.12)',border:'rgba(22,163,74,0.50)',arrow:'url(#lmArrGreen)',label:'Ресурси та пули'}
};
var lmMob=window.innerWidth<=768;
var nodes = [
    {id:'coroutines',title:'Корутини',desc:'Базова одиниця асинхронності — запуск паралельних задач',
     ...
     url:'/uk/docs/components/coroutines.html',group:'primitives',order:1,w:130,h:44},
    {id:'future',title:'Future',desc:'Отримання результату асинхронної операції',
     ...
     url:'/uk/docs/components/future.html',group:'primitives',order:null,w:110,h:44},
    {id:'await-funcs',title:'await, await_all',desc:'Очікування однієї або кількох корутин або Future',
     ...
     url:'/uk/docs/reference/await.html',group:'sync',order:2,w:160,h:44},
    {id:'channels',title:'Канали',desc:'Передача даних між корутинами',
     ...
     url:'/uk/docs/components/channels.html',group:'sync',order:null,w:110,h:44},
    {id:'cancellation',title:'Скасування',desc:'Скасування корутин',
     code:'<span class="var">$coro</span>-><span class="fn">cancel</span>();\n<span class="cm">// або з таймаутом:</span>\n<span class="fn">await</span>(<span class="var">$coro</span>, <span class="fn">timeout</span>(<span class="num">5000</span>));',
     url:'/uk/docs/components/cancellation.html',group:'cancellation',order:3,w:150,h:44},
    {id:'scope',title:'Scope',desc:'Контроль часу життя групи корутин',
     ...
     url:'/uk/docs/components/scope.html',group:'structural',order:4,w:110,h:44},
    {id:'taskgroup',title:'TaskGroup',desc:'Структурна конкурентність: групування задач з гарантованим очікуванням або скасуванням',
     ...
     url:'/uk/docs/components/task-group.html',group:'structural',order:null,w:130,h:44},
{id:'taskset',title:'TaskSet',desc:'Динамічний набір задач з автоочищенням результатів після доставки',code:'<span class="var">$set</span> = <span class="kw">new</span> <span class="fn">TaskSet</span>(<span class="num">10</span>);
<span class="var">$set</span>-><span class="fn">spawn</span>(<span class="var">$task</span>);
<span class="var">$r</span> = <span class="var">$set</span>-><span class="fn">joinNext</span>();',url:'/uk/docs/components/task-set.html',group:'structural',order:null,w:120,h:44},
    {id:'context',title:'Контекст',desc:'Зберігання даних, прив\'язаних до корутини (напр., токен авторизації)',
     ...
     url:'/uk/docs/components/context.html',group:'context',order:5,w:120,h:44},
    {id:'iterate',title:'iterate()',desc:'Конкурентна обробка колекцій',
     ...
     url:'/uk/docs/reference/iterate.html',group:'iterate',order:null,w:120,h:44},
    {id:'pool',title:'Async\\Pool',desc:'Повторне використання дорогих ресурсів (з\'єднань, воркерів)',
     ...
     url:'/uk/docs/components/pool.html',group:'resources',order:6,w:130,h:44},
    {id:'pdo-pool',title:'PDO Pool',desc:'Пул з\'єднань PDO до бази даних. Використовуйте звичні функції прозоро',
     ...
     url:'/uk/docs/components/pdo-pool.html',group:'resources',order:null,w:120,h:44}
];
...
var subPages = {
    'coroutines':[
        {label:'spawn()',url:'/uk/docs/reference/spawn.html'},
        {label:'spawn_with()',url:'/uk/docs/reference/spawn-with.html'}],
    'future':[{label:'await()',url:'/uk/docs/reference/await.html'}],
    'await-funcs':[
        {label:'await_all()',url:'/uk/docs/reference/await-all.html'},
        {label:'await_first_success()',url:'/uk/docs/reference/await-first-success.html'},
        {label:'delay()',url:'/uk/docs/reference/delay.html'},
        {label:'suspend()',url:'/uk/docs/reference/suspend.html'}],
    'cancellation':[
        {label:'cancel()',url:'/uk/docs/components/cancellation.html'},
        {label:'protect()',url:'/uk/docs/reference/protect.html'},
        {label:'timeout()',url:'/uk/docs/reference/timeout.html'}],
    'scope':[
        {label:'Scope',url:'/uk/docs/components/scope.html'},
        {label:'Scope::inherit()',url:'/uk/docs/components/scope.html'}],
    'context':[
        {label:'current_context()',url:'/uk/docs/reference/current-context.html'},
        {label:'coroutine_context()',url:'/uk/docs/reference/coroutine-context.html'}],
    'taskgroup':[
        {label:'all()',url:'/uk/docs/reference/task-group/all.html'},
        {label:'race()',url:'/uk/docs/reference/task-group/race.html'},
        {label:'seal()',url:'/uk/docs/reference/task-group/seal.html'},
        {label:'cancel()',url:'/uk/docs/reference/task-group/cancel.html'}],
'taskset':[
        {label:'joinNext()',url:'/uk/docs/reference/task-set/join-next.html'},
        {label:'joinAny()',url:'/uk/docs/reference/task-set/join-any.html'},
        {label:'joinAll()',url:'/uk/docs/reference/task-set/join-all.html'},
        {label:'seal()',url:'/uk/docs/reference/task-set/seal.html'}],
    'pool':[{label:'Pool::tryAcquire()',url:'/uk/docs/components/pool.html'}]
};
var dZones=[
    {group:'primitives',label:'Базові примітиви',x:140,y:10,w:400,h:85,rx:14},
    {group:'sync',label:'Синхронізація',x:100,y:105,w:440,h:85,rx:14},
    {group:'cancellation',label:'Скасування',x:170,y:200,w:260,h:85,rx:14},
    {group:'structural',label:'Стр. конкурентність',x:140,y:295,w:470,h:85,rx:14},
    {group:'context',label:'Контекст',x:185,y:390,w:230,h:85,rx:14},
    {group:'iterate',label:'iterate()',x:30,y:485,w:190,h:90,rx:14},
    {group:'resources',label:'Ресурси та пули',x:245,y:585,w:330,h:90,rx:14}
];
var mZones=[
    {group:'primitives',label:'Базові примітиви',x:15,y:10,w:350,h:90,rx:14},
    {group:'sync',label:'Синхронізація',x:15,y:108,w:350,h:90,rx:14},
    {group:'cancellation',label:'Скасування',x:65,y:205,w:250,h:85,rx:14},
    {group:'structural',label:'Стр. конкурентність',x:15,y:300,w:350,h:190,rx:14},
    {group:'context',label:'Контекст',x:65,y:498,w:250,h:85,rx:14},
    {group:'iterate',label:'iterate()',x:15,y:595,w:175,h:90,rx:14},
    {group:'resources',label:'Resources & Pools',x:155,y:595,w:220,h:190,rx:14}
];
...
function showTip(id,ev){
    ...
    if(n.order){oe.textContent='Крок '+n.order+' з 6';oe.style.color=g.color;oe.style.display='';}
...
if(lmIsMobile){document.getElementById('lmHint').textContent='Натисніть на вузол для деталей.';}
