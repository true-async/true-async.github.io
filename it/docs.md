...
<p class="lm-hint" id="lmHint">Passa il mouse su un nodo per i dettagli. Clicca per andare alla documentazione.</p>
...
<button class="sh-close" id="lmShClose" aria-label="Chiudi">&times;</button>
</div>
...
<a class="sh-btn" id="lmShBtn" href="#">Apri documentazione &rarr;</a>
</div>
<a class="lm-link" href="/it/interactive/learning-map.html">Versione interattiva completa &rarr;</a>
<script>
(function(){
var GRP = {
    primitives:   {color:'#6B58FF',bg:'rgba(107,88,255,0.12)',border:'rgba(107,88,255,0.50)',arrow:'url(#lmArrPurple)',label:'Primitive di base'},
    sync:         {color:'#2563EB',bg:'rgba(37,99,235,0.12)',border:'rgba(37,99,235,0.50)',arrow:'url(#lmArrBlue)',label:'Sincronizzazione'},
    cancellation: {color:'#DC2626',bg:'rgba(220,38,38,0.12)',border:'rgba(220,38,38,0.50)',arrow:'url(#lmArrRed)',label:'Cancellazione'},
    structural:   {color:'#0891B2',bg:'rgba(8,145,178,0.12)',border:'rgba(8,145,178,0.50)',arrow:'url(#lmArrTeal)',label:'Conc. Strutturata'},
    ...
    resources:    {color:'#16A34A',bg:'rgba(22,163,74,0.12)',border:'rgba(22,163,74,0.50)',arrow:'url(#lmArrGreen)',label:'Risorse e Pool'}
};
var lmMob=window.innerWidth<=768;
var nodes = [
    {id:'coroutines',title:'Coroutines',desc:'Unita base dell\'asincronia -- lancio di task concorrenti',
     ...
     url:'/it/docs/components/coroutines.html',group:'primitives',order:1,w:130,h:44},
    {id:'future',title:'Future',desc:'Ottenere il risultato di un\'operazione asincrona',
     ...
     url:'/it/docs/components/future.html',group:'primitives',order:null,w:110,h:44},
    {id:'await-funcs',title:'await, await_all',desc:'Attendere una o piu coroutine o Future',
     ...
     url:'/it/docs/reference/await.html',group:'sync',order:2,w:160,h:44},
    {id:'channels',title:'Channels',desc:'Passare dati tra coroutine',
     ...
     url:'/it/docs/components/channels.html',group:'sync',order:null,w:110,h:44},
    {id:'cancellation',title:'Cancellation',desc:'Cancellare le coroutine',
     code:'<span class="var">$coro</span>-><span class="fn">cancel</span>();\n<span class="cm">// o con timeout:</span>\n<span class="fn">await</span>(<span class="var">$coro</span>, <span class="fn">timeout</span>(<span class="num">5000</span>));',
     url:'/it/docs/components/cancellation.html',group:'cancellation',order:3,w:150,h:44},
    {id:'scope',title:'Scope',desc:'Controllare il ciclo di vita di un gruppo di coroutine',
     ...
     url:'/it/docs/components/scope.html',group:'structural',order:4,w:110,h:44},
    {id:'taskgroup',title:'TaskGroup',desc:'Concorrenza strutturata: raggruppare task con await o cancellazione garantiti',
     ...
     url:'/it/docs/components/task-group.html',group:'structural',order:null,w:130,h:44},
{id:'taskset',title:'TaskSet',desc:'Insieme dinamico di task con pulizia automatica dopo la consegna',code:'<span class="var">$set</span> = <span class="kw">new</span> <span class="fn">TaskSet</span>(<span class="num">10</span>);
<span class="var">$set</span>-><span class="fn">spawn</span>(<span class="var">$task</span>);
<span class="var">$r</span> = <span class="var">$set</span>-><span class="fn">joinNext</span>();',url:'/it/docs/components/task-set.html',group:'structural',order:null,w:120,h:44},
    {id:'context',title:'Context',desc:'Memorizzare dati legati a una coroutine (es. token di autenticazione)',
     ...
     url:'/it/docs/components/context.html',group:'context',order:5,w:120,h:44},
    {id:'iterate',title:'iterate()',desc:'Elaborazione concorrente delle collezioni',
     ...
     url:'/it/docs/reference/iterate.html',group:'iterate',order:null,w:120,h:44},
    {id:'pool',title:'Async\\Pool',desc:'Riutilizzare risorse costose (connessioni, worker)',
     ...
     url:'/it/docs/components/pool.html',group:'resources',order:6,w:130,h:44},
    {id:'pdo-pool',title:'PDO Pool',desc:'Pool di connessioni database PDO. Usa le funzioni familiari in modo trasparente',
     ...
     url:'/it/docs/components/pdo-pool.html',group:'resources',order:null,w:120,h:44}
];
...
var subPages = {
    'coroutines':[
        {label:'spawn()',url:'/it/docs/reference/spawn.html'},
        {label:'spawn_with()',url:'/it/docs/reference/spawn-with.html'}],
    'future':[{label:'await()',url:'/it/docs/reference/await.html'}],
    'await-funcs':[
        {label:'await_all()',url:'/it/docs/reference/await-all.html'},
        {label:'await_first_success()',url:'/it/docs/reference/await-first-success.html'},
        {label:'delay()',url:'/it/docs/reference/delay.html'},
        {label:'suspend()',url:'/it/docs/reference/suspend.html'}],
    'cancellation':[
        {label:'cancel()',url:'/it/docs/components/cancellation.html'},
        {label:'protect()',url:'/it/docs/reference/protect.html'},
        {label:'timeout()',url:'/it/docs/reference/timeout.html'}],
    'scope':[
        {label:'Scope',url:'/it/docs/components/scope.html'},
        {label:'Scope::inherit()',url:'/it/docs/components/scope.html'}],
    'context':[
        {label:'current_context()',url:'/it/docs/reference/current-context.html'},
        {label:'coroutine_context()',url:'/it/docs/reference/coroutine-context.html'}],
    'taskgroup':[
        {label:'all()',url:'/it/docs/reference/task-group/all.html'},
        {label:'race()',url:'/it/docs/reference/task-group/race.html'},
        {label:'seal()',url:'/it/docs/reference/task-group/seal.html'},
        {label:'cancel()',url:'/it/docs/reference/task-group/cancel.html'}],
'taskset':[
        {label:'joinNext()',url:'/it/docs/reference/task-set/join-next.html'},
        {label:'joinAny()',url:'/it/docs/reference/task-set/join-any.html'},
        {label:'joinAll()',url:'/it/docs/reference/task-set/join-all.html'},
        {label:'seal()',url:'/it/docs/reference/task-set/seal.html'}],
    'pool':[{label:'Pool::tryAcquire()',url:'/it/docs/components/pool.html'}]
};
var dZones=[
    {group:'primitives',label:'Primitive di base',x:140,y:10,w:400,h:85,rx:14},
    {group:'sync',label:'Sincronizzazione',x:100,y:105,w:440,h:85,rx:14},
    {group:'cancellation',label:'Cancellazione',x:170,y:200,w:260,h:85,rx:14},
    {group:'structural',label:'Conc. Strutturata',x:140,y:295,w:470,h:85,rx:14},
    {group:'context',label:'Context',x:185,y:390,w:230,h:85,rx:14},
    {group:'iterate',label:'iterate()',x:30,y:485,w:190,h:90,rx:14},
    {group:'resources',label:'Risorse e Pool',x:245,y:585,w:330,h:90,rx:14}
];
var mZones=[
    {group:'primitives',label:'Primitive di base',x:15,y:10,w:350,h:90,rx:14},
    {group:'sync',label:'Sincronizzazione',x:15,y:108,w:350,h:90,rx:14},
    {group:'cancellation',label:'Cancellazione',x:65,y:205,w:250,h:85,rx:14},
    {group:'structural',label:'Conc. Strutturata',x:15,y:300,w:350,h:190,rx:14},
...
function showTip(id,ev){
    ...
    if(n.order){oe.textContent='Passo '+n.order+' di 6';oe.style.color=g.color;oe.style.display='';}
...
if(lmIsMobile){document.getElementById('lmHint').textContent='Tocca un nodo per i dettagli.';}
...
`PHP TrueAsync` è disponibile in tre varianti:
* Immagine Docker
* Pacchetti precompilati per le distribuzioni più diffuse
* Codice sorgente per la compilazione manuale
