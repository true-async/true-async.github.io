...
path_key: "/docs.html"
nav_active: docs
permalink: /zh/docs.html
page_title: "文档"
description: "TrueAsync 文档。了解如何安装和使用 PHP 的真正异步原语。"
---

## 简介 {#introduction}

`PHP TrueAsync` 是一个通过修改 Zend 内核、I/O 库、数据库库、套接字库以及其他函数来实现 PHP 真正异步的项目。
...
<p class="lm-hint" id="lmHint">将鼠标悬停在节点上查看详情。点击可跳转到文档。</p>
...
<button class="sh-close" id="lmShClose" aria-label="关闭">&times;</button>
</div>
...
<a class="sh-btn" id="lmShBtn" href="#">打开文档 &rarr;</a>
</div>
<a class="lm-link" href="/zh/interactive/learning-map.html">完整交互版本 &rarr;</a>
<script>
(function(){
var GRP = {
    primitives:   {color:'#6B58FF',bg:'rgba(107,88,255,0.12)',border:'rgba(107,88,255,0.50)',arrow:'url(#lmArrPurple)',label:'基础原语'},
    sync:         {color:'#2563EB',bg:'rgba(37,99,235,0.12)',border:'rgba(37,99,235,0.50)',arrow:'url(#lmArrBlue)',label:'同步'},
    cancellation: {color:'#DC2626',bg:'rgba(220,38,38,0.12)',border:'rgba(220,38,38,0.50)',arrow:'url(#lmArrRed)',label:'取消'},
    structural:   {color:'#0891B2',bg:'rgba(8,145,178,0.12)',border:'rgba(8,145,178,0.50)',arrow:'url(#lmArrTeal)',label:'结构化并发'},
    context:      {color:'#8B5CF6',bg:'rgba(139,92,246,0.12)',border:'rgba(139,92,246,0.50)',arrow:'url(#lmArrViolet)',label:'上下文'},
    iterate:      {color:'#F59E0B',bg:'rgba(245,158,11,0.12)',border:'rgba(245,158,11,0.50)',arrow:'url(#lmArrAmber)',label:'iterate()'},
    resources:    {color:'#16A34A',bg:'rgba(22,163,74,0.12)',border:'rgba(22,163,74,0.50)',arrow:'url(#lmArrGreen)',label:'资源与连接池'}
};
var lmMob=window.innerWidth<=768;
var nodes = [
    {id:'coroutines',title:'协程',desc:'异步的基本单元 — 启动并发任务',
    ...
    {id:'future',title:'Future',desc:'获取异步操作的结果',
    ...
    {id:'await-funcs',title:'await, await_all',desc:'等待一个或多个协程或 Future',
    ...
    {id:'channels',title:'通道',desc:'在协程之间传递数据',
    ...
    {id:'iterate',title:'iterate()',desc:'并发集合处理',
    ...
    {id:'pool',title:'Async\\Pool',desc:'复用昂贵的资源（连接、工作进程）',
    ...
    {id:'pdo-pool',title:'PDO Pool',desc:'PDO 数据库连接池。透明地使用熟悉的函数',
...
var subPages = {
    ...
    'future':[{label:'await()',url:'/zh/docs/reference/await.html'}],
    'await-funcs':[
    ...
    'taskgroup':[
        {label:'all()',url:'/zh/docs/reference/task-group/all.html'},
        {label:'race()',url:'/zh/docs/reference/task-group/race.html'},
        {label:'close()',url:'/zh/docs/reference/task-group/close.html'},
    ...
    'taskset':[
        ...
        {label:'close()',url:'/zh/docs/reference/task-set/close.html'}],
    'pool':[{label:'Pool::tryAcquire()',url:'/zh/docs/components/pool.html'}]
};
var dZones=[
    {group:'primitives',label:'基础原语',x:140,y:10,w:400,h:85,rx:14},
    {group:'sync',label:'同步',x:100,y:105,w:440,h:85,rx:14},
    {group:'cancellation',label:'取消',x:170,y:200,w:260,h:85,rx:14},
    {group:'structural',label:'结构化并发',x:140,y:295,w:470,h:85,rx:14},
    {group:'context',label:'上下文',x:185,y:390,w:230,h:85,rx:14},
    {group:'iterate',label:'iterate()',x:30,y:485,w:190,h:90,rx:14},
    {group:'resources',label:'资源与连接池',x:245,y:585,w:330,h:90,rx:14}
];
var mZones=[
    {group:'primitives',label:'基础原语',x:15,y:10,w:350,h:90,rx:14},
    {group:'sync',label:'同步',x:15,y:108,w:350,h:90,rx:14},
    {group:'cancellation',label:'取消',x:65,y:205,w:250,h:85,rx:14},
    {group:'structural',label:'结构化并发',x:15,y:300,w:350,h:190,rx:14},
    {group:'context',label:'上下文',x:65,y:498,w:250,h:85,rx:14},
...
function showTip(id,ev){
    ...
    if(n.order){oe.textContent='第 '+n.order+' 步，共 6 步';oe.style.color=g.color;oe.style.display='';}
...
if(lmIsMobile){document.getElementById('lmHint').textContent='点击节点查看详情。';}
