...
> **现已提供原生方案。** 自 2026-05 起，TrueAsync 拥有自家的服务器 ——
> [**TrueAsync Server**](/zh/docs/server/index.html)。它是一个 PHP 扩展，直接在 PHP 进程内
> 提供 HTTP/1.1/2/3，不依赖 Go 运行时和 Caddy。如果你需要 Caddy 的能力（自动 Let's Encrypt、
> Caddyfile 路由），或者要接入现有的 Caddy 基础设施，FrankenPHP 依然适用。
