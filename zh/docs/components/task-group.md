...
|-------------------------|-------------------------------------|---------------------------------|------------------------------------------|---------------------------|
| 结构化并发              | `close()` + `all()->await()`         | `async with` 块                | `try-with-resources` + `join()`          | 通过作用域自动实现         |
...
- [TaskGroup::close](/zh/docs/reference/task-group/close.html) -- 密封组，不再接受新任务
