...
Для автодополнения, встроенной документации и стабов для статического анализа установите dev-пакет [`true-async/ide-helper`](https://github.com/true-async/ide-helper). Он покрывает async-ядро, HTTP-сервер и клиент ClickHouse и работает с PhpStorm, PHPStan и Psalm.
