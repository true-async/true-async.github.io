...
path_key: "/docs.html"
nav_active: docs
permalink: /ko/docs.html
page_title: "문서"
description: "TrueAsync 문서. PHP를 위한 진정한 비동기 프리미티브를 설치하고 사용하는 방법을 알아보세요."
---

## 소개 {#introduction}

`PHP TrueAsync`는 Zend 코어, I/O 라이브러리,
데이터베이스 라이브러리, 소켓 라이브러리 및 기타 함수를 수정하여 PHP에서 진정한 비동기를 구현하는 프로젝트입니다.
...
<p class="lm-hint" id="lmHint">노드 위에 마우스를 올리면 상세 정보를 볼 수 있습니다. 클릭하면 문서로 이동합니다.</p>
...
<button class="sh-close" id="lmShClose" aria-label="닫기">&times;</button>
</div>
...
<a class="sh-btn" id="lmShBtn" href="#">문서 열기 &rarr;</a>
</div>
<a class="lm-link" href="/ko/interactive/learning-map.html">전체 인터랙티브 버전 &rarr;</a>
<script>
(function(){
var GRP = {
    primitives:   {color:'#6B58FF',bg:'rgba(107,88,255,0.12)',border:'rgba(107,88,255,0.50)',arrow:'url(#lmArrPurple)',label:'기본 프리미티브'},
    sync:         {color:'#2563EB',bg:'rgba(37,99,235,0.12)',border:'rgba(37,99,235,0.50)',arrow:'url(#lmArrBlue)',label:'동기화'},
    cancellation: {color:'#DC2626',bg:'rgba(220,38,38,0.12)',border:'rgba(220,38,38,0.50)',arrow:'url(#lmArrRed)',label:'취소'},
    structural:   {color:'#0891B2',bg:'rgba(8,145,178,0.12)',border:'rgba(8,145,178,0.50)',arrow:'url(#lmArrTeal)',label:'구조적 동시성'},
    context:      {color:'#8B5CF6',bg:'rgba(139,92,246,0.12)',border:'rgba(139,92,246,0.50)',arrow:'url(#lmArrViolet)',label:'컨텍스트'},
    iterate:      {color:'#F59E0B',bg:'rgba(245,158,11,0.12)',border:'rgba(245,158,11,0.50)',arrow:'url(#lmArrAmber)',label:'iterate()'},
    resources:    {color:'#16A34A',bg:'rgba(22,163,74,0.12)',border:'rgba(22,163,74,0.50)',arrow:'url(#lmArrGreen)',label:'리소스 및 풀'}
};
var lmMob=window.innerWidth<=768;
var nodes = [
    {id:'coroutines',title:'코루틴',desc:'비동기의 기본 단위 — 동시 작업 실행',
     ...
     url:'/ko/docs/components/coroutines.html',group:'primitives',order:1,w:130,h:44},
    {id:'future',title:'Future',desc:'비동기 작업의 결과 얻기',
     ...
     url:'/ko/docs/components/future.html',group:'primitives',order:null,w:110,h:44},
    {id:'await-funcs',title:'await, await_all',desc:'하나 이상의 코루틴 또는 Future 대기',
     ...
     url:'/ko/docs/reference/await.html',group:'sync',order:2,w:160,h:44},
    {id:'channels',title:'채널',desc:'코루틴 간 데이터 전달',
     ...
     url:'/ko/docs/components/channels.html',group:'sync',order:null,w:110,h:44},
    {id:'cancellation',title:'취소',desc:'코루틴 취소',
     code:'<span class="var">$coro</span>-><span class="fn">cancel</span>();\n<span class="cm">// 또는 타임아웃과 함께:</span>\n<span class="fn">await</span>(<span class="var">$coro</span>, <span class="fn">timeout</span>(<span class="num">5000</span>));',
     url:'/ko/docs/components/cancellation.html',group:'cancellation',order:3,w:150,h:44},
    {id:'scope',title:'Scope',desc:'코루틴 그룹의 수명 제어',
     ...
     url:'/ko/docs/components/scope.html',group:'structural',order:4,w:110,h:44},
    ...
    {id:'iterate',title:'iterate()',desc:'동시 컬렉션 처리',
     ...
     url:'/ko/docs/reference/iterate.html',group:'iterate',order:null,w:120,h:44},
    {id:'pool',title:'Async\\Pool',desc:'비용이 큰 리소스 재사용 (연결, 워커)',
     ...
     url:'/ko/docs/components/pool.html',group:'resources',order:6,w:130,h:44},
    {id:'pdo-pool',title:'PDO Pool',desc:'PDO 데이터베이스 연결 풀. 익숙한 함수를 투명하게 사용',
     ...
     url:'/ko/docs/components/pdo-pool.html',group:'resources',order:null,w:120,h:44}
];
...
var subPages = {
    'coroutines':[
        {label:'spawn()',url:'/ko/docs/reference/spawn.html'},
        {label:'spawn_with()',url:'/ko/docs/reference/spawn-with.html'}],
    'future':[{label:'await()',url:'/ko/docs/reference/await.html'}],
    'await-funcs':[
        {label:'await_all()',url:'/ko/docs/reference/await-all.html'},
        {label:'await_first_success()',url:'/ko/docs/reference/await-first-success.html'},
        {label:'delay()',url:'/ko/docs/reference/delay.html'},
        {label:'suspend()',url:'/ko/docs/reference/suspend.html'}],
    'cancellation':[
        {label:'cancel()',url:'/ko/docs/components/cancellation.html'},
        {label:'protect()',url:'/ko/docs/reference/protect.html'},
        {label:'timeout()',url:'/ko/docs/reference/timeout.html'}],
    'scope':[
        {label:'Scope',url:'/ko/docs/components/scope.html'},
        {label:'Scope::inherit()',url:'/ko/docs/components/scope.html'}],
    'context':[
        {label:'current_context()',url:'/ko/docs/reference/current-context.html'},
        {label:'coroutine_context()',url:'/ko/docs/reference/coroutine-context.html'}],
    'taskgroup':[
        {label:'all()',url:'/ko/docs/reference/task-group/all.html'},
        {label:'race()',url:'/ko/docs/reference/task-group/race.html'},
        {label:'close()',url:'/ko/docs/reference/task-group/close.html'},
    ...
    'taskset':[
        ...
        {label:'close()',url:'/ko/docs/reference/task-set/close.html'}],
    'pool':[{label:'Pool::tryAcquire()',url:'/ko/docs/components/pool.html'}]
};
var dZones=[
    {group:'primitives',label:'기본 프리미티브',x:140,y:10,w:400,h:85,rx:14},
    {group:'sync',label:'동기화',x:100,y:105,w:440,h:85,rx:14},
    {group:'cancellation',label:'취소',x:170,y:200,w:260,h:85,rx:14},
    {group:'structural',label:'구조적 동시성',x:140,y:295,w:470,h:85,rx:14},
    {group:'context',label:'컨텍스트',x:185,y:390,w:230,h:85,rx:14},
    {group:'iterate',label:'iterate()',x:30,y:485,w:190,h:90,rx:14},
    {group:'resources',label:'리소스 및 풀',x:245,y:585,w:330,h:90,rx:14}
];
var mZones=[
    {group:'primitives',label:'기본 프리미티브',x:15,y:10,w:350,h:90,rx:14},
    {group:'sync',label:'동기화',x:15,y:108,w:350,h:90,rx:14},
    {group:'cancellation',label:'취소',x:65,y:205,w:250,h:85,rx:14},
    {group:'structural',label:'구조적 동시성',x:15,y:300,w:350,h:190,rx:14},
    {group:'context',label:'컨텍스트',x:65,y:498,w:250,h:85,rx:14},
...
function showTip(id,ev){
    ...
    if(n.order){oe.textContent=n.order+'단계 / 6단계';oe.style.color=g.color;oe.style.display='';}
...
if(lmIsMobile){document.getElementById('lmHint').textContent='노드를 탭하면 상세 정보를 볼 수 있습니다.';}
