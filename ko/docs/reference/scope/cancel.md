...
해당 스코프에 속한 모든 코루틴을 취소합니다. 각 활성 코루틴은 `AsyncCancellation`을 받게 됩니다. `$cancellationError`가 지정되면 취소 사유로 사용됩니다.

## 매개변수

`cancellationError` — 사용자 정의 취소 예외. `null`이면 표준 `AsyncCancellation`이 사용됩니다.
