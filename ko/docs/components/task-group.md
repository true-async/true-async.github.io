...
`TaskGroup`은 `close()` 메서드를 사용하여 봉인할 때까지
동적으로 작업을 추가할 수 있습니다.
...
이 경우 모든 작업을 추가한 후 `close()`을 호출하여
...
|-------------------------|-------------------------------------|---------------------------------|------------------------------------------|---------------------------|
| 구조적 동시성            | `close()` + `all()->await()`         | `async with` 블록               | `try-with-resources` + `join()`          | 스코프를 통해 자동         |
...
- [TaskGroup::close](/ko/docs/reference/task-group/close.html) -- 새 작업에 대해 그룹 봉인
...
- [TaskGroup::isClosed](/ko/docs/reference/task-group/is-closed.html) -- 그룹 봉인 여부 확인
