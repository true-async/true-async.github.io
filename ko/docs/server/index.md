...
WebSocket과 SSE는 이미 완성되어 동일한 단일 listener + 프로토콜 감지 모델로 동작합니다.
HTTP/2 위의 gRPC는 아직 작업 중입니다([Roadmap](#기능) 참고).
...
| ✅ | **WebSocket** | RFC 6455, HTTP/1.1 및 HTTP/2에서의 Upgrade(RFC 8441 Extended CONNECT), `wss://`, permessage-deflate(RFC 7692), 전이중, backpressure, Autobahn|Testsuite 246개 테스트 전부 |
| ✅ | **SSE** | HTTP/1.1, HTTP/2, HTTP/3 위의 `text/event-stream`, 프로토콜에 관계없이 동일한 핸들러 |
