...
nav_active: docs
permalink: /en/docs/server/examples.html
page_title: "TrueAsync Server: examples"
description: "Ready-made recipes: JSON API, fan-out, multipart upload, static, redirect, SSE, bailout firewall."
---

# TrueAsync Server examples

(PHP 8.6+, true_async_server 0.6+)

## JSON API with parallel fan-out
...
$server->addHttpHandler(function ($req, $res) {
    ...
    // Three independent database queries in parallel
...
// /assets/* is served from public/, no PHP handler
...
## Multipart upload with file move
...
## Streaming a large upload without holding it in RAM
...
$server->addHttpHandler(function ($req, $res) use ($cache) {
    ...
    if ($cached !== null) {
        // a string → sent as is, without re-packing
        $res->json($cached);
        return;
    }
...
## Bailout firewall: fatals do not bring the server down

A deliberately broken handler:
...
A request to `/boom` produces **500 Internal Server Error**, the handler coroutine drains, and the
listener keeps accepting new connections. The same behaviour applies to E_ERROR, OOM, and uncaught
exceptions during shutdown. Works on H1, H2, and H3.

## Custom HTTP exception

`TrueAsync\HttpException extends Async\AsyncCancellation`. Throw it anywhere inside the handler to
send a specific HTTP status through the normal cancellation chain.
...
The status is taken from `$code` (must be 4xx/5xx, otherwise 500) and the body from `$message`.

## See also

- [`examples/` in the repository](https://github.com/true-async/server/tree/main/examples)
