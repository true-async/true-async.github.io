...
- `Async\OperationCanceledException` — if the cancellation token triggered (including timeout). The original exception from the token is available via `$e->getPrevious()` (e.g., `TimeoutException` when using `timeout()`).
