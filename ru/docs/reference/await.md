...
## Как передаётся исключение

Когда корутина завершается с исключением, **результат «оседает» на её дескрипторе** до тех пор,
пока кто-то его не заберёт. Поведение симметрично `Async\Future` и зависит от того, удерживает ли
кто-нибудь дескриптор корутины кроме Scheduler:

- **Дескриптор удерживается** (`$coro = spawn(...)`, корутина лежит в массиве, передана в
  `await_all()` и т.п.) — исключение хранится на дескрипторе и ждёт. Любой `await($coro)` его получит,
  даже если корутина уже давно завершилась.
- **Дескриптор никем не удерживается** (fire-and-forget — `spawn(...)` без сохранения результата) —
  исключение проявляется при разрушении handle через fire-and-forget safety net.

Главное практическое следствие — **`await` ловит исключение даже при гонке**:
...
// Корутина может завершиться раньше, чем мы дойдём до await — это нормально.
// Исключение спокойно дождётся нас здесь:
try {
    await($coro);
} catch (RuntimeException $e) {
    echo "поймал: ", $e->getMessage(), "\n"; // поймал: boom
}
```

То же касается `await_all()`, `await_any_or_fail()` и других `await_*()`: можно собрать корутины
в массив, дать им поработать параллельно и потом дожидаться. Исключения соберутся через `await`.

> Когда parent-scope умирает раньше своей корутины, дочерние корутины получают `AsyncCancellation`
> по спецификации. Эта ветка обрабатывается отдельно и не зависит от того, кто удерживает дескриптор.
