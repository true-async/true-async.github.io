...
копируются в persistent-память при создании потока, а на той стороне снова в emalloc-heap child-потока.
...
- **Класс замыкания** — `$task` должен быть `\Closure`. Callables других типов (`[object, 'method']`,
  строковое имя функции) не принимаются — транспорт умеет копировать только Closure через механизм
  op-array snapshot'а.
