...
- Задачи, ожидающие в очереди (ещё не взятые рабочим потоком), **немедленно отклоняются** — соответствующие объекты `Future` переходят в состояние отклонения с `ThreadPoolException` (а в режиме `coroutine: true` — с `CancellationException`).
- В **обычном режиме** (`coroutine: false`) задачи, уже выполняющиеся в рабочих потоках, доводятся до конца текущей задачи — принудительное прерывание PHP-кода в OS-потоке невозможно.
- В **режиме `coroutine: true`** in-flight задачи **реально кэнселятся** (начиная с TrueAsync 0.7.0): атомарный флаг `cancel_requested` выставляется до закрытия канала, воркер вызывает `ZEND_ASYNC_SCOPE_CANCEL(pool_scope, NULL, false, false)`, и `AFTER_MAIN` каскадно отменяет все child-scope'ы запущенных задач.
- Рабочие потоки останавливаются сразу после завершения текущей задачи и не берут новые задачи из очереди.

Для **мягкого** завершения, при котором все задачи в очереди доводятся до конца, используйте [`close()`](/ru/docs/reference/thread-pool/close.html).
