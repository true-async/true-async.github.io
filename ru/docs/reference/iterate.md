...
> Исключения из callback-ов не распространяются на родительский Scope — они передаются
> непосредственно вызывающему коду `iterate()`.
...
> **Примечание:** Для `Traversable`-объектов `iterate()` гарантирует безопасное
> изменение позиции итератора — новые корутины не порождаются,
> пока итератор перемещается к следующему элементу.
