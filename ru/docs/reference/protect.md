...
На время выполнения `$closure` корутина помечается как защищённая. Если в этот момент поступает запрос на отмену, `AsyncCancellation` будет выброшен только **после** завершения замыкания.
...
// AsyncCancellation будет выброшен здесь — после commit()
?>
```
...
$coroutine = spawn(function() {
    ...
    // AsyncCancellation выбрасывается здесь — после protect()
...
> **Примечание:** Если отмена произошла во время `protect()`, `AsyncCancellation` будет выброшен сразу после возврата из замыкания — результат `protect()` в этом случае потеряется.
