...
В этот момент выбрасывается `Cancellation`, и корутина завершает своё выполнение.
...
Корутина, которая игнорирует отмену, превращается в зомби:
она потребляет ресурсы, а её результат никому не нужен.

Если вам нужно выполнить очистку при отмене — используйте `finally`:
...
Частный случай отмены — таймаут. Функция `timeout()` создаёт ограничение по времени:
...
class QueueWorker {
    ...
    public function start(): void {
        spawn with $this->scope function() {
            while (true) {
                $job = $this->queue->take();

                protect(function() use ($job) {
                    $job->process();
                    $job->markDone();
                });
            }
...
При вызове `stop()` воркер завершает текущую задачу благодаря `protect()`,
но новые задачи из очереди уже не берёт.
