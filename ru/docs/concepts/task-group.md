...
## TaskGroup::all(), запечатывание и асинхронный итератор 

`TaskGroup::all()` возвращает `Future`, который срабатывает в момент времени, когда все существующие задачи
в очереди завершены. Однако, `TaskGroup` позволяет добавлять задачи снова и снова, до тех пор он не будет
запечатан с помощью метода `seal()`. Это позволяет использовать `TaskGroup` в цикле, где задачи добавляются динамически,
а `all()` вызывается для получения результатов текущего набора задач.

`TaskGroup` может использоваться вместе с `foreach` для итерации по результатам по мере их готовности. 
