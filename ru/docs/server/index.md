...
WebSocket и SSE уже готовы и работают через ту же модель одного listener'а с детектом
протокола. gRPC поверх HTTP/2 пока в работе (см. [Roadmap](#возможности)).
...
| ✅ | **WebSocket** | RFC 6455, Upgrade с HTTP/1.1 и HTTP/2 (RFC 8441 Extended CONNECT), `wss://`, permessage-deflate (RFC 7692), full-duplex, backpressure, все 246 тестов Autobahn\|Testsuite |
| ✅ | **SSE** | `text/event-stream` поверх HTTP/1.1, HTTP/2 и HTTP/3: один и тот же обработчик независимо от протокола |
