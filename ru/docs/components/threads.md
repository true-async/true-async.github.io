...
PHP-request'ом**: у него свой `EG()`, свой heap, свой op-array, свой autoloader. Между потоками ничего
не разделяется по ссылке — любые данные перекочёвывают через **глубокое копирование**.
...
Потоки **тяжёлые** — создание требует поднять TSRM-слот, запустить request_startup, скопировать замыкание
и его captured-переменные в persistent-память. Поэтому их не спавнят тысячами: обычная модель — несколько
worker-потоков (по числу CPU-ядер) либо один поток для изолирующей операции.
...
| `Closure` (замыкания)               | Специальный snapshot: op-array + bound variables        |
