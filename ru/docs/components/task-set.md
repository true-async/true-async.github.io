...
`TaskGroup` отлично подходит для сценариев, когда количество задач известно заранее
и все результаты нужны одновременно. Но в реальных приложениях часто встречается другой паттерн:
задачи приходят динамически, результаты обрабатываются по мере готовности,
а обработанные записи должны автоматически освобождать память.

Типичные примеры:

- **Worker-цикл**: сервер принимает запросы и порождает задачи, обрабатывая результаты по мере готовности.
- **Pipeline обработки**: результаты одного этапа передаются следующему без накопления.
- **Мониторинг**: периодически добавляются проверки, завершённые сразу убираются из набора.

Использование `TaskGroup` для таких сценариев приводит к тому, что завершённые записи
остаются в памяти до вызова `all()` или `getResults()`, а `count()` не отражает
реального количества активных задач.

**TaskSet** решает эти проблемы. Это специализированная версия `TaskGroup`,
в которой завершённые записи автоматически удаляются в момент доставки результата:
через `joinNext()`, `joinAll()`, `joinAny()` или `foreach`.
...
Это **главное концептуальное отличие** TaskSet от TaskGroup.
