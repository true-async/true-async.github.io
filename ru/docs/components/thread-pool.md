...
Замыкание `$bootloader` deep-copy'ится один раз и выполняется в **каждом** воркере перед task-loop'ом.
Идеальное место для autoload, прогрева пулов соединений, прекомпиляции opcache — всего, что иначе
запускалось бы внутри каждого `submit()`.

**Брошенное в bootloader исключение фейлит весь пул**: воркер не стартует, ошибка поднимается
в родителя.

### Coroutine-mode

С `coroutine: true` каждая задача выполняется **как корутина** в собственном child-scope под
per-worker pool scope. Внутри задачи можно делать `await`, использовать `Channel`-ы, IO, `spawn` —
всё без блокировки воркера.
