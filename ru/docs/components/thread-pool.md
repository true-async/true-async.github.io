...
```php
// автодетект по числу CPU
$pool = new ThreadPool();

// явное число воркеров
$pool = new ThreadPool(workers: 4);

// + размер очереди
$pool = new ThreadPool(workers: 4, queueSize: 64);

// + bootloader, выполняемый в каждом воркере один раз
$pool = new ThreadPool(
    workers: 4,
    queueSize: 64,
    bootloader: function () {
...
// + coroutine-mode: задача стартует как корутина внутри per-worker pool scope
$pool = new ThreadPool(workers: 4, coroutine: true);
```

| Параметр     | Тип            | Назначение                                                         | По умолчанию    |
|--------------|----------------|--------------------------------------------------------------------|-----------------|
| `$workers`   | `int`          | Число воркер-потоков. `0` — автодетект через `available_parallelism()` | `0`         |
| `$queueSize` | `int`          | Максимальная длина очереди ожидающих задач                         | `workers × 4`   |
| `$bootloader`| `?\Closure`    | Per-worker startup hook (см. ниже)                                 | `null`          |
| `$coroutine` | `bool`         | Запускать каждую задачу как корутину (см. ниже)                    | `false`         |
| `$concurrency`| `int`         | Лимит одновременных корутин на воркер (только при `coroutine: true`)| `0` (без лимита)|

Все воркер-потоки запускаются **сразу при создании** пула. Это небольшое «авансовое» вложение,
зато последующие `submit()` не несут накладных расходов на запуск потока.
...
### Автодетект числа воркеров

Когда `workers: 0` (или параметр опущен), пул берёт значение из
[`Async\available_parallelism()`](/ru/docs/reference/available-parallelism.html). Эта функция
учитывает `cgroup`-квоты, affinity и контейнерные лимиты — на Kubernetes-podе с `cpu.max=2`
получите 2, а не физическое число ядер хоста.

### Bootloader

Замыкание `$bootloader` один раз глубоко копируется и выполняется в **каждом** воркере перед
основным циклом обработки задач. Идеальное место для autoload, прогрева пулов соединений и
прекомпиляции opcache — всего, что иначе запускалось бы внутри каждого `submit()`.

Если bootloader бросит исключение, весь пул считается несостоявшимся: воркер не стартует, а
ошибка поднимается в родителя.

### Режим корутин

С `coroutine: true` каждая задача выполняется **как корутина** в своей дочерней области (scope),
которая в свою очередь вложена в общую область пула воркера. Внутри задачи можно делать `await`,
использовать `Channel`-ы, IO и `spawn` — всё без блокировки самого воркера.

```php
$pool = new ThreadPool(workers: 4, coroutine: true);

$pool->submit(function () {
    // обычные блокирующие вызовы в этом режиме корректно паркуют корутину,
    // а не блокируют OS-поток
    $rows = (new PDO('mysql:...'))->query('SELECT ...')->fetchAll();
    return $rows;
});
```

`$concurrency` ограничивает число одновременно живых корутин внутри **одного** воркера.
