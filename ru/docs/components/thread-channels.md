...
`Async\Channel` работает **внутри одного потока** — между корутинами одного планировщика. Данные
в нём живут в **локальной памяти потока**, и безопасность обеспечивается тем, что одновременно к каналу
обращается только одна корутина.

`Async\ThreadChannel` предназначен для передачи данных **между OS-потоками**. Буфер канала живёт
в **общей памяти**, доступной всем потокам, а не в памяти какого-то одного из них. Каждое отправленное
значение глубоко копируется в эту общую память, а на стороне получателя — обратно в локальную память
его потока. Синхронизация — через потокобезопасный мьютекс, так что `send()` и `recv()` можно вызывать
из разных OS-потоков одновременно.

| Свойство                          | `Async\Channel`          | `Async\ThreadChannel`            |
|-----------------------------------|--------------------------|----------------------------------|
| Область видимости                 | Один OS-поток            | Между OS-потоками                |
| Где лежат буферизированные данные | Локальная память потока  | Общая память, видимая всем потокам |
| Синхронизация                     | Планировщик корутин (кооперативно) | Мьютекс (потокобезопасно) |
...
Producer пишет в канал из отдельного потока, основной поток читает через `recv()` — ничего специального,
...
приостановка — другие корутины этого планировщика продолжают работать), пока кто-то из других потоков
не освободит место.

Значение **глубоко копируется в общую память канала** по тем же правилам, что и переменные, захваченные
через `use(...)` в `spawn_thread()`. Объекты с динамическими свойствами, ссылки PHP, ресурсы
отклоняются с `Async\ThreadTransferException`.
...
- **Класс `ThreadChannel` не поддерживает сериализацию.** Объекты канала нельзя сохранить в файл или передать по сети —
