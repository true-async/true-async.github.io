...
`delay` бросила исключение `Async\AsyncCancellation`. Интересно, что данный трюк не сработает с `sleep(1)`, 
так как `sleep(1)` не бросает исключение, а `delay` бросает, чем мы и воспользовались.
