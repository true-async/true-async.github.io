...
Классическая модель `FrankenPHP` (и не только): один `PHP`-поток — один запрос.
Корутины открывают возможность создания Web-сервера, который сможет обрабатывать с помощью одной PHP-VM
сразу несколько запросов. Давайте сделаем это!
