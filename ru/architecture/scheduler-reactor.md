...
Нет гарантии, что в будущем алгоритм очереди не будет изменён. При этом иногда возникают редкие моменты, 
когда приоритет корутин имеет значение.
...
* `true` — `handler` остаётся и будет вызван снова.
* `false` — `Scheduler` удалит его. 
