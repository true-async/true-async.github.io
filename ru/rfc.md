...
Проект TrueAsync около года продвигался через официальный процесс `RFC` на wiki.php.net.
...
На текущий проект `TrueAsync` столкнулся с неопределённостью в процессе `RFC`.
За последние несколько месяцев обсуждение практически остановилось, и нет ясности относительно его будущего.
Вполне очевидно, что `RFC` не сможет пройти голосование, и при этом нет никакой возможности что-то изменить.

По этим причинам на текущий момент процесс `RFC` считается замороженным,
и проект будет развиваться в рамках открытого сообщества, без "официального" статуса.
